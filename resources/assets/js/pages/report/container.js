const VC = function () {
    ...
    const initPie = async function (datapie, datanames) {
        ...
        VC.properties.pie.setOption({
            ...
            // Add title
            title: {
                text: 'Weight By Restaurant',
                subtext: 'Restaurant Information',
                left: 'center',
                textStyle: {
                    fontSize: 17,
                    fontWeight: 500
    ...
    const getByContainer = async function (start, end) {
        start.startOf('day');
        end.endOf('day');
        let data = await BioReactor.getByContainer({
    ...
    const convertData = async function(data) {
        ...
        const result = data.reduce((acc, element) => {
            const found = acc.find(a => a.name == element.user);
            if (!found){
                names.push(element.user);
                acc.push({value: element.diffWeight, name: element.user}); 
            } else {
    ...
    const initReport = async function(start, end) {
        let data = await getByContainer(start, end);
